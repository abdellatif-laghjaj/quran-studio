  ...
  const renderFrame = () => {
    ...
    if (verses.length > 0 && fontsLoaded) {
      ...
      if (config.showTranslation && displayedTranslation) {
        ...
        const finalTranslation = isPageMode
          ? displayedTranslation.toUpperCase()
          : displayedTranslation;
        const finalFont = isPageMode
          ? `italic 600 ${config.translationFontSize}px serif`
          : `bold ${config.translationFontSize}px "${config.translationFontFamily}"`;
        ...
        for (let n = 0; n < transWords.length; n++) {
          ...
          if (metrics.width > transMaxWidth && n > 0) {
            transLines.push(transLine);
            transLine = transWords[n] + " ";
          } else {
            transLine = testLine;
          }
        }
        ...
        if (translationStart !== undefined && translationEnd !== undefined) {
          ...
          if (duration > 0) {
            progress = Math.max(
              0,
              Math.min(1, (currentTimeMs - translationStart) / duration),
            );
          }
        } else {
          progress = 1;
        ...
        const totalChars = transLines.reduce(
        ...
        transLines.forEach((l, i) => {
          ...
          if (currentHighlightMode === "none") {
            ...
            if (
              currentHighlightMode === "karaoke" ||
              currentHighlightMode === "word"
            ) {
  ...
  return (
    <div className="relative shadow-2xl rounded-2xl overflow-hidden border-2 border-studio-border ring-1 ring-white/5 bg-studio-bg group transition-colors duration-300">
      ...
      {(isLoading || (!fontsLoaded && verses.length > 0)) && (
        <div
          dir="ltr"
          className="absolute inset-0 bg-studio-bg/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4"
        >
          ...
          <div className="flex flex-col items-center">
            <span className="text-studio-text font-medium text-lg">
              {!fontsLoaded ? "Loading Fonts..." : "Loading Resources"}
            </span>
            <span className="text-studio-textMuted text-sm">
              {loadingProgress}
            </span>
          </div>
        </div>
      ...
      {isExporting && (
        <div
          dir="ltr"
          className="absolute inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center gap-6 z-50"
        >
          <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-xs font-bold text-white">
                {verses.length > 0
                  ? Math.round(((currentVerseIndex + 1) / verses.length) * 100)
                  : 0}
                %
              </span>
            </div>
          ...
          <div className="text-center">
            <h3 className="text-2xl font-bold text-white mb-2 bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">
              Rendering Video
            </h3>
            <p className="text-sm text-studio-textMuted">
              Rendering Verse {currentVerseIndex + 1} of {verses.length}
            </p>
          </div>
        </div>
      )}
