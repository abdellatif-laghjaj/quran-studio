...
interface RenderContext {
  ...
  streamLayoutRef: React.MutableRefObject<{
    words: StreamWordLayout[];
    configSig: string;
  } | null>;
...
export const renderStreamMode = ({
  ...
  if (
    ...
    streamLayoutRef.current.configSig !== configSig
  ) {
    ...
    verses.forEach((verse, vIdx) => {
      ...
      for (let i = 0; i < verse.words.length; i++) {
        ...
        if (segment) {
          ...
          const prev = layoutWords[layoutWords.length - 1];
          if (prev) {
            start = prev.endTime;
            end = prev.endTime + 500;
          }
        }
        if (parts.length > 1) {
          ...
          if (
            nextVerse &&
            nextVerse.timing &&
            nextVerse.timing.segments.length > 0
          ) {
  ...
  const foundIndex = allWords.findIndex(
  ...
  if (foundIndex !== -1) activeLayoutIndex = foundIndex;
    ...
    allWords.forEach((w, idx) => {
      const diff = Math.min(
        Math.abs(currentTimeMs - w.startTime),
        Math.abs(currentTimeMs - w.endTime),
      );
      if (diff < minDiff) {
        minDiff = diff;
  ...
  if (activeWord) {
    ...
    if (Math.abs(targetOffset - cameraOffsetRef.current) > 2000)
  ...
  allWords.forEach((layout, i) => {
    ...
    const visibleThreshold = canvas.width * 0.4;
    const fadeStart = config.fontSize * 1.5;
    let opacity = 1;
    if (distFromCenter > fadeStart)
      opacity =
        1 - (distFromCenter - fadeStart) / (visibleThreshold - fadeStart);
    ...
    if (opacity > 0.01) {
      ...
      if (isActive) {
        ...
        layout.parts.forEach((part) => {
          ctx.font = part.font;
          const isHighlight =
            isActive && config.enableHighlight && part.type === "text";
...
export const renderSingleMode = ({
  ...
  words.forEach((word, index) => {
    ...
    if (
      (currentChunk.length >= WORDS_PER_LINE && !nextIsEnd) ||
      isEnd ||
  ...
  if (activeVerse?.timing?.segments && activeVerse.timing.segments.length > 0) {
    ...
    if (activeSegment) activeWordPosition = activeSegment[0];
      ...
      else if (currentTimeMs < segments[0][1])
        activeWordPosition = segments[0][0];
      else {
        const prevSegment = [...segments]
          .sort((a, b) => b[2] - a[2])
          .find((s) => s[2] <= currentTimeMs);
        if (prevSegment) activeWordPosition = prevSegment[0];
      }
    }
  }
  chunks.forEach((chunk, cIdx) => {
    if (chunk.some((w) => w.position === activeWordPosition))
  ...
  const measuredWords = chunkToRender.map((word) => {
    ...
    return {
      ...
      displayWord,
      font: fontStr,
      width: ctx.measureText(displayWord).width,
    };
  });

  const totalWidth = measuredWords.reduce(
    (acc, w, i) =>
      acc + w.width + (i < measuredWords.length - 1 ? globalWordSpacing : 0),
    0,
  );
  ...
  measuredWords.forEach((word) => {
    ...
    if (config.enableHighlight && activeVerse?.timing?.segments) {
      const segment = activeVerse.timing.segments.find(
        (s) => s[0] === word.position,
      );
      if (segment) {
        const [_, start, end] = segment;
        if (currentTimeMs >= start && currentTimeMs < end) isActive = true;
      }
    }
    ...
    if (isActive) {
      ...
      if (word.char_type_name === "end")
        ctx.fillStyle = config.verseNumberColor;
      else ctx.fillStyle = config.textColor;
  ...
  return {
    text: chunkToRender
      ...
      .join(" "),
    start:
      chunkToRender.length > 0 && activeVerse?.timing?.segments
        ? activeVerse.timing.segments.find(
            (s) => s[0] === chunkToRender[0].position,
          )?.[1]
        : undefined,
    end:
      chunkToRender.length > 0 && activeVerse?.timing?.segments
        ? activeVerse.timing.segments.find(
            (s) => s[0] === chunkToRender[chunkToRender.length - 1].position,
          )?.[2]
        : undefined,
  };
};
...
export const renderPageMode = ({
  ...
  for (let i = 0; i < currentVerseWords.length; i++) {
    ...
    const currentLineWidth = currentLineWords.reduce(
      (acc, w) => acc + w.effectiveWidth,
      0,
    );
  ...
  lines.forEach((line, lineIndex) => {
    ...
    const totalLineWidth = line.words.reduce(
      (acc, w) => acc + w.effectiveWidth,
      0,
    );
