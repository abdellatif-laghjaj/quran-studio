  ...
  useEffect(() => {
    const loadPageFonts = async () => {
      ...
      if (verses.length > 0) {
        ...
        if (fontType === "v4") {
          const familyNames = Array.from(uniquePages).map(
            (p) => `"quran-v4-p${p}"`,
          );
        ...
        const promises = Array.from(uniquePages).map(async (page) => {
          ...
          const baseUrls =
            FONT_BASE_URLS[fontType as keyof typeof FONT_BASE_URLS];
          ...
          for (const baseUrl of baseUrls) {
            ...
            try {
              ...
              console.warn(
                `Failed to load font ${fontFamilyName} from ${baseUrl}`,
                e,
              );
            }
          }
