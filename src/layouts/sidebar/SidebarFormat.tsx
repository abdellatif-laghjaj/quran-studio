...
export default function SidebarFormat({
  config,
  setConfig,
}: SidebarFormatProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex p-1 bg-[#1a1a1e] rounded-xl border border-white/5">
        {[
          ...
          <button
            key={mode.id}
            onClick={() =>
              setConfig({ ...config, visualizationMode: mode.id as any })
            }
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <label className="text-[10px] uppercase text-studio-textMuted font-bold tracking-wide">
            Theme
          </label>
          ...
          <div className="flex gap-1 p-1 bg-[#1a1a1e] rounded-lg border border-white/5">
            {[
              ...
              <button
                key={ratio.id}
                onClick={() =>
                  setConfig({ ...config, aspectRatio: ratio.id as any })
                }
                className={`flex-1 flex items-center justify-center py-1.5 rounded-md transition-all ${
