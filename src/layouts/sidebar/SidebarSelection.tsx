...
function VerseDropdown({
  ...
  const options = Array.from({ length: max }, (_, i) => i + 1).filter(
    (n) => n >= min,
  );
  ...
  return (
    <div className="flex-1 relative group" ref={containerRef}>
      ...
      >
        ...
        <span className="text-sm font-bold text-white font-mono flex-1 text-center">
          {value}
        </span>
        <ChevronDown
          className={`w-3 h-3 text-zinc-600 transition-transform duration-200 ${isOpen ? "rotate-180 text-studio-accent" : ""}`}
        />
      </div>
        ...
        <div className="relative z-50">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <ChevronDown
                className={`w-4 h-4 text-zinc-500 transition-transform duration-300 ${isSurahOpen ? "rotate-180 text-studio-accent" : ""}`}
              />
            </div>
          </button>
                ...
                {filteredSurahs.map((s) => (
                  ...
                  >
                    <div className="flex items-center gap-3 text-left">
                      <span
                        className={`text-base font-lateef w-8 text-center ${config.surahId === s.number ? "text-white/80" : "text-zinc-500 group-hover:text-zinc-400"}`}
                      >
                        {s.number}
                      </span>
                      <div className="flex flex-col items-start">
                        <span
                          className={`text-base font-lateef font-bold ${config.surahId === s.number ? "text-white" : "text-zinc-200 group-hover:text-white"}`}
                        >
                          {s.englishName}
                        </span>
                        <span
                          className={`text-[10px] uppercase tracking-wider ${config.surahId === s.number ? "text-white/60" : "text-zinc-600 group-hover:text-zinc-500"}`}
                        >
                          {s.totalAyahs} Ayahs
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`font-lateef text-2xl ${config.surahId === s.number ? "text-white/90" : "text-zinc-400 group-hover:text-zinc-300"}`}
                      >
                        {s.arabicName}
                      </span>
                      {s.number === 97 && (
