    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-[#1a1a1e] rounded-lg p-0.5 border border-white/5">
          <button
            onClick={() => setTypoTab("quran")}
            className={`px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all ${typoTab === "quran" ? "bg-white/10 text-white" : "text-zinc-500"}`}
          >
            Quran
          </button>
          <button
            onClick={() => setTypoTab("translation")}
            className={`px-3 py-1 text-[10px] font-bold uppercase rounded-md transition-all ${typoTab === "translation" ? "bg-white/10 text-white" : "text-zinc-500"}`}
          >
            Sub
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            {config.surahId === 97 && (
              <button
                onClick={() =>
                  setConfig({ ...config, fontFamily: AL_QADR_FONT.family })
                }
          ...
          <div className="p-3 bg-[#1a1a1e] border border-white/5 rounded-xl space-y-4">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setConfig({ ...config, quranTextY: parseInt(e.target.value) })
                }
                className="w-full accent-studio-accent h-1.5 bg-black/50 rounded-lg appearance-none cursor-pointer"
              />
            </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {ENGLISH_FONTS.slice(0, 6).map((font) => (
              <button
                key={font.name}
                onClick={() =>
                  setConfig({ ...config, translationFontFamily: font.family })
                }
